...
func (_q *SummaryQuery) QueryPost() *PostQuery {
...
func (_q *SummaryQuery) QuerySource() *SourceQuery {
...
func (_q *SummaryQuery) WithPost(opts ...func(*PostQuery)) *SummaryQuery {
	query := (&PostClient{config: _q.config}).Query()
...
func (_q *SummaryQuery) WithSource(opts ...func(*SourceQuery)) *SummaryQuery {
	query := (&SourceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SummaryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Summary, error) {
	...
	if query := _q.withPost; query != nil {
		if err := _q.loadPost(ctx, query, nodes, nil,
			func(n *Summary, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSource; query != nil {
		if err := _q.loadSource(ctx, query, nodes, nil,
			func(n *Summary, e *Source) { n.Edges.Source = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SummaryQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Summary, init func(*Summary), assign func(*Summary, *Post)) error {
...
func (_q *SummaryQuery) loadSource(ctx context.Context, query *SourceQuery, nodes []*Summary, init func(*Summary), assign func(*Summary, *Source)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Summary)
	for i := range nodes {
		fk := nodes[i].SourceID
...
func (_q *SummaryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPost != nil {
			_spec.Node.AddColumnOnce(summary.FieldPostID)
		}
		if _q.withSource != nil {
			_spec.Node.AddColumnOnce(summary.FieldSourceID)
		}
	}
